...
module.exports = function(server, dispatch, callback) {
  ...
  server.get('/idea/:ideaId/comment/:commentId', function(req, res, next) {
    dispatch.emit('action:getComment', req.params.ideaId, req.params.commentId, req, res);
    // Stop handling routes here
    return next(false);
  });

  server.post('/idea/:ideaId/comment', function(req, res, next) {
    dispatch.emit('action:postComment', req.params.ideaId, req.body, req, res);
    // Stop handling routes here
    return next(false);
  });

  server.get('/idea/:ideaId', function(req, res, next) {
    dispatch.emit('action:getIdea', req.params.ideaId, req, res);
    // Stop handling routes here
    return next(false);
  });

  server.post('/idea', function(req, res, next) {
    dispatch.emit('action:postIdea', req.body, req, res);
    // Stop handling routes here
    return next(false);
  });
